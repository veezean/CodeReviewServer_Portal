...
export default {
  ...
  methods: {
    loadProjects() {
      const queryDeptId = this.search.queryParams.deptId
        ? this.search.queryParams.deptId
        : -1;
    ...
    deleteSelection(val: any) {
      ElMessageBox.confirm("确定要删除所选项目吗？此操作不可恢复！", "Warning", {
        ...
        .then(() => {
          ...
          api
            .get(`server/project/deleteProjects?projectIds=${projectIds}`)
            .then((resp: any) => {
              if (resp.code === 0) {
                ElMessage({
                  type: "success",
    ...
    deleteSingle(val: { id: any }) {
      ElMessageBox.confirm("确定要删除此项目吗？此操作不可恢复！", "Warning", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
        .then(() => {
          api
            .get(`server/project/deleteProject?projectId=${val.id}`)
            .then((resp: any) => {
              if (resp.code === 0) {
                ElMessage({
                  type: "success",
    ...
    memberManage(val: any) {
      api
        .get(`server/project/queryProjectMembers?projectId=${val.id}`)
        .then((resp: any) => {
          this.memberEditDetail = {
            userMembers: resp.data,
            projectId: `${val.id}`,
          };
    ...
    saveMemberOperation(val: any) {
      (this.$refs.memberDetailForm as any).validate((valid: any) => {
        if (!valid) {
          ElMessage({
            type: "error",
    ...
    cancelMemberOperation(val: any) {
      this.memberDialog = false;
    },

    editSingle(val: any) {
      api
        .get(`server/project/queryProject?projectId=${val.id}`)
        .then((resp: any) => {
          this.editDetail = {
    ...
    saveEditOperation() {
      (this.$refs.editDetailForm as any).validate((valid: any) => {
        if (!valid) {
          ...
          api
            .post(
              `server/project/modifyProject?projId=${(this.editDetail as any).id}`,
              this.editDetail
            )
            .then((resp: any) => {
              if (resp.code === 0) {
    ...
    saveCreateOperation() {
      (this.$refs.createDetailForm as any).validate((valid: any) => {
        if (!valid) {
          ...
          api
            .post("server/project/createProject", this.createDetail)
            .then((resp: any) => {
              if (resp.code === 0) {
                ElMessage({
                  type: "success",
...
<template>
  <div>
    <page-header
      title="项目管理"
      content="本页面提供对系统内所有的项目信息进行维护，项目需要绑定到部门，支持按照部门维度筛选自己部门内的项目列表。"
    />
          ...
          <el-row>
            <el-col :span="6">
              <el-form-item label="归属部门">
                <el-tree-select
                  clearable
                  v-model="search.queryParams.deptId"
                  :data="departmentTree"
      ...
      >
        ...
        <el-table-column fixed="right" label="操作" width="200">
          <template #default="scope">
            <el-button link type="primary" size="small" @click="memberManage(scope.row)">
              成员管理
            </el-button>
    ...
    <el-dialog v-model="showEditDialog" title="编辑项目信息">
      ...
      >
        ...
        <el-col :span="18">
          <el-form-item
            label="项目名称"
            prop="projectName"
            :rules="editFieldRules.projectName"
          >
            <el-input
              v-model="(editDetail as any).projectName"
              placeholder="输入项目的名称"
              maxlength="64"
              show-word-limit
            />
        ...
        <el-col :span="24">
          <el-form-item label="归属部门">
            <el-tree-select
              v-model="(editDetail as any).departmentId"
              :data="departmentTree"
              :render-after-expand="false"
              check-strictly="true"
            />
        ...
        <el-col :span="18">
          <el-form-item label="项目描述">
            <el-input
              v-model="(editDetail as any).projectDesc"
              type="textarea"
              :rows="3"
              placeholder="输入项目的简要描述信息"
              clearable
              maxlength="32"
              show-word-limit
            />
      ...
      <template #footer>
        <span class="dialog-footer">
          ...
          <el-button type="primary" @click="saveEditOperation"> 保存 </el-button>
        </span>
      </template>
    </el-dialog>
    ...
    <el-dialog v-model="showCreateDialog" title="创建项目信息">
      ...
      >
        <el-col :span="18">
          ...
          >
            <el-input
              v-model="(createDetail as any).projectName"
              placeholder="输入项目的名称"
              maxlength="64"
              show-word-limit
            />
        ...
        <el-col :span="24">
          <el-form-item label="归属部门">
            <el-tree-select
              v-model="(createDetail as any).departmentId"
              :data="departmentTree"
              :render-after-expand="false"
              check-strictly="true"
            />
        ...
        <el-col :span="18">
          <el-form-item label="项目描述">
            <el-input
              v-model="(createDetail as any).projectDesc"
              type="textarea"
              :rows="3"
              placeholder="输入项目的简要描述信息"
              clearable
    ...
    <el-dialog v-model="memberDialog" title="管理项目成员">
      <el-form
        ref="memberDetailForm"
        :model="memberEditDetail"
        size="default"
        label-width="120px"
      >
        <el-col :span="24">
      ...
      <template #footer>
        <span class="dialog-footer">
          <el-button @click="cancelMemberOperation"> 取消 </el-button>
          <el-button type="primary" @click="saveMemberOperation"> 保存 </el-button>
        </span>
      </template>
    </el-dialog>
