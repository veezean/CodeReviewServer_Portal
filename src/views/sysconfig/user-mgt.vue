...
export default {
  ...
  methods: {
    ...
    deleteSelection(val: any) {
      ElMessageBox.confirm("确定要删除所选用户吗？此操作不可恢复！", "Warning", {
    ...
    deleteSingle(val: { account: any }) {
      ElMessageBox.confirm("确定要删除此用户吗？此操作不可恢复！", "Warning", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
    ...
    editSingle(val: any) {
      api
        .get(`server/user/getUserDetail?account=${val.account}`)
        .then((resp: any) => {
          this.editUserDetail = resp.data;
          this.selectedRoles = [];
          for (
            let index = 0;
            index < (this.editUserDetail as any).roles.length;
            index++
          ) {
            (this.selectedRoles as any).push(
              (this.editUserDetail as any).roles[index].id
            );
          }
...
<template>
  <div>
    <page-header
      title="用户管理"
      content="本页面提供对系统内用户数据的整体维护管理能力，可以新增用户或者删除用户操作，或者对已有用户信息进行变更。"
    />
          ...
          <el-row>
            <el-col :span="6">
              <el-form-item label="姓名">
                <el-input
                  v-model="search.queryParams.name"
            ...
            <el-col :span="6">
              <el-form-item label="账号">
                <el-input
                  v-model="search.queryParams.account"
            ...
            <el-col :span="6">
              <el-form-item label="手机号">
                <el-input
                  v-model="search.queryParams.phoneNumber"
                  placeholder="手机号查询"
                  clearable
                />
              </el-form-item>
    ...
    <el-dialog v-model="showEditDialog" title="编辑用户信息">
      <el-form
        ref="editUserDetailForm"
        :model="editUserDetail"
        size="default"
        label-width="120px"
      ...
      >
        <el-col :span="18">
          <el-form-item label="账号">
            <el-input
              v-model="(editUserDetail as any).account"
              placeholder="用户账号"
              readonly
              disabled
            />
        ...
        <el-col :span="18">
          <el-form-item label="姓名" prop="name" :rules="editFieldRules.name">
            <el-input
              v-model="(editUserDetail as any).name"
              placeholder="用户姓名"
              maxlength="64"
              show-word-limit
            />
        ...
        <el-col :span="18">
          <el-form-item label="手机号">
            <el-input
              v-model="(editUserDetail as any).phoneNumber"
              placeholder="手机号"
              clearable
              maxlength="32"
              show-word-limit
        ...
        <el-col :span="24">
          <el-form-item label="部门">
            <el-tree-select
              v-model="(editUserDetail as any).department.id"
              :data="departmentTree"
              :render-after-expand="false"
              check-strictly
            />
    ...
    <el-dialog v-model="showCreateDialog" title="新建用户">
      <el-form
        ref="createUserDetailForm"
        :model="createUserDetail"
        size="default"
        label-width="120px"
        :rules="editFieldRules"
      >
        <el-col :span="18">
          <el-form-item label="账号" prop="account" :rules="editFieldRules.account">
            <el-input
              v-model="(createUserDetail as any).account"
              placeholder="输入用户账号"
              maxlength="64"
              show-word-limit
            />
        ...
        <el-col :span="18">
          <el-form-item label="姓名" prop="name" :rules="editFieldRules.name">
            <el-input
              v-model="(createUserDetail as any).name"
              placeholder="输入用户姓名"
              maxlength="64"
              show-word-limit
            />
        ...
        <el-col :span="18">
          <el-form-item label="手机号">
            <el-input
              v-model="(createUserDetail as any).phoneNumber"
              placeholder="手机号"
              clearable
              maxlength="32"
              show-word-limit
        ...
        <el-col :span="24">
          <el-form-item label="部门">
            <el-tree-select
              v-model="(createUserDetail as any).departmentId"
              :data="departmentTree"
              :render-after-expand="false"
              check-strictly
            />
