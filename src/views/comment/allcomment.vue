  ...
  methods: {
    ...
    deleteSelection(val) {
      ElMessageBox.confirm(
        ...
        .then(() => {
          ...
          api.get(`server/comment/deleteComments?identifiers=${identifiers}`).then((resp) => {
            if (resp.code === 0) {
              ElMessage({
                type: 'success',
                message: '删除成功',
    ...
    deleteSingle(val: { identifier: any }) {
      ElMessageBox.confirm(
        ...
        .then(() => {
          api.get(`server/comment/deleteComment?identifier=${val.identifier}`).then((resp) => {
            if (resp.code === 0) {
              ElMessage({
                type: 'success',
                message: '删除成功',
    ...
    editSingle(val) {
      api.get(`server/comment/initEditReqBody?identifier=${val.identifier}`).then((resp) => {
        this.editDetail = resp.data
          ...
          <el-row>
            ...
            <el-col :span="6">
              <el-form-item label="指派确认人员">
                <el-select v-model.trim="search.queryParams.assignConfirmUser" placeholder="请输入姓名或账号查询" clearable filterable>
                  <el-option
                    v-for="(item, index) in listUsers"
                    :key="item.account"
                    :label="item.name"
            ...
            <el-col :span="6">
              <el-form-item label="实际确认人员">
                <el-select v-model.trim="search.queryParams.realConfirmUser" placeholder="请输入姓名或账号查询" clearable filterable>
                  <el-option
                    v-for="(item, index) in listUsers"
                    :key="item.account"
                    :label="item.name"
            ...
            <el-col :span="6">
              <el-form-item label="确认结果">
                <el-select v-model.trim="search.queryParams.confirmResult" placeholder="输入评审意见的确认状态" clearable>
                  <el-option
      ...
      <el-table border highlight-current-row :data="commentsData.list" height="100%" @selection-change="getSelectedRows">
        ...
        <el-table-column fixed="right" label="操作" width="200">
          <template #default="scope">
            ...
            <el-button link type="primary" size="small" @click="confirmSingle(scope.row)">
              确认
            </el-button>
